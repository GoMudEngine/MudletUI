function ui.updateInvDisplay()
	...
	if gmcp.Char.Inventory == nil then
		return
	end
	if gmcp.Char.Inventory.Backpack == nil then
		return
	end
	if gmcp.Char.Inventory.Backpack.items == nil then
		return
	end
	...
	if #backpackItems > 0 then
		for _, item in ipairs(backpackItems) do
			ui.eqDisplay:cechoPopup("Inventory", "<sandy_brown>  " .. ui.titleCase(item.name) .. "\n", {
