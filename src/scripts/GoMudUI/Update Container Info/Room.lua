...
function ui.updateRoomDisplay()
	...
	if not (room and room.Info) then
		return
	end
	...
	if info.quest == 0 then
		...
		ui.roomDisplay:cecho(
			"Room",
			"<dodger_blue>Quest<white>    : <gold>"
				.. info.quest
				.. " <white>("
				.. ui.questStatusColor[info.queststatus]
				.. info.queststatus
				.. "<white>)\n"
		)
		ui.roomDisplay:cecho("Room", "<dodger_blue>QName<white>    : <reset>")
		ui.roomDisplay:cechoLink(
			"Room",
	...
	ui.roomDisplay:cecho(
		"Room",
		"<ForestGreen>Adventurers<white>:<reset> "
			.. adventureList
			.. "\n<cyan>NPC's <white>:<reset> "
			.. npcList
			.. "\n<green>Items <white>: <reset>"
			.. itemList
			.. "\n\n"
	)
