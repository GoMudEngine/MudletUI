function ui.updateEQDisplay()
	if not gmcp.Char or not gmcp.Char.Inventory then
		return
	end
	...
	for _, slot in ipairs(locations) do
		...
		-- Check Wielded first
		if
			gmcp.Char.Inventory.Wielded
			and gmcp.Char.Inventory.Wielded[slot]
			and gmcp.Char.Inventory.Wielded[slot].name ~= "-nothing-"
		then
			item = gmcp.Char.Inventory.Wielded[slot]
		elseif
			gmcp.Char.Inventory.Worn
			and gmcp.Char.Inventory.Worn[slot]
			and gmcp.Char.Inventory.Worn[slot].name ~= "-nothing-"
		then
		...
		if not item then
			...
			ui.eqDisplay:cechoPopup("Equipment", "<sandy_brown>" .. ui.titleCase(item.name), {
