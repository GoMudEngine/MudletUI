function ui.updatePromptDisplay()
	...
	if not ui.promptDisplay then
		return
	end

	-- Cache values with sensible defaults.
	local xp, xptnl = 0, 1000
	local energy, energyMax = 100, 100
	local xpPct, xpPctPretty = 0, 0
	local gold, bank = 0, 0
	local carry, capacity = 0, 10
	--local spell, charge = "", ""

	if gmcp.Char and gmcp.Char.Worth then
	...
	if gmcp.Char and gmcp.Char.Inventory and gmcp.Char.Inventory.Backpack then
	...
	disp:cecho(
		" <DarkSeaGreen>[<white>EN<gold>:<green> "
	...
	disp:cecho(
		" <DarkSeaGreen>[<white>XP<gold>:<green> "
			.. ui.addNumberSeparator(xp)
			.. "<grey>/<grey>"
			.. ui.addNumberSeparator(xptnl)
			.. " <gold>"
			.. xpPctPretty
			.. "<white>%<grey> TNL<DarkSeaGreen>] <grey>-"
	)

	disp:cecho(
		" <DarkSeaGreen>[<white>Gold<gold>:<gold> "
	...
	disp:cecho(
		" <DarkSeaGreen>[<DarkTurquoise>Carry<white>: <grey>"
			.. carry
			.. "<white>/<grey>"
			.. capacity
			.. "<DarkSeaGreen>]"
