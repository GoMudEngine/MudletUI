...
function ui.updateRoomDisplay()
	local hasRoomInfo = ui.hasGmcpData("Room", "Info")

	ui.updateDisplay("roomDisplay", "Room", function(display, tabName)
		if not hasRoomInfo then
			-- Show default room display
			display:cecho(tabName, ui.createHeader("Room", "The Void", display:get_width()) .. "\n")
			display:cecho(tabName, "<white>Area<gold>: <DarkSeaGreen>Unknown\n")
			display:cecho(tabName, "<white>Environment<gold>: <grey>Unknown\n\n")
			display:cecho(tabName, "<grey>Waiting for location data...\n\n")
			display:cecho(tabName, "<white>Exits<gold>: <grey>none")
			return
		end
