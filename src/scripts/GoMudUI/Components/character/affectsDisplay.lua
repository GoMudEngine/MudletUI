function ui.showTempAffects(name, duration, isBuff)
	...
	local affect = color
		...
		.. string.rep(
			" ",
			math.ceil(ui.affectsDisplay:get_width() / ui.consoleFontWidth)
				- 20
				- string.len(name)
				- (4 - string.len(name))
				- string.len(timeLen)
...
function ui.updateAffectsDisplay()
	if not ui.hasGmcpData("Char", "Affects") then
		ui.affectsDisplay:clear("Affects")
		ui.affectsDisplay:cecho("Affects", ui.createHeader("Affects", "None", ui.affectsDisplay:get_width()) .. "\n\n")
		ui.affectsDisplay:cecho("Affects", "<grey>No active affects")
		return
	end
	...
	ui.affectsDisplay:cecho(
		"Affects",
		"<white>Affected by:"
			.. string.rep(" ", math.floor(ui.affectsDisplay:get_width() / ui.consoleFontWidth) - 22)
			.. "Duration: "
	)
	...
	for _, affect in pairs(gmcp.Char.Affects) do
		...
		local duration = tonumber(affect.duration_cur) or 0
		...
		if isBuff then
			...
			if isPermanent then
				...
				table.insert(ui.affectsTable.buff.timed, ui.showTempAffects(affect.name, duration, true))
			end
		else
			if isPermanent then
				table.insert(ui.affectsTable.debuff.permanent, affect.name)
			else
				table.insert(ui.affectsTable.debuff.timed, ui.showTempAffects(affect.name, duration, false))
			end
		end
	end
	...
	for _, affect in pairs(ui.affectsTable.buff.permanent) do
		ui.affectsDisplay:cecho(
			...
			"<SkyBlue>"
				...
				.. string.rep(
					" ",
					math.floor(ui.affectsDisplay:get_width() / ui.consoleFontWidth) - string.len(affect) - 3
				)
				.. "<gold>-- \n"
		)
	end
	...
	if haveBuff then
		ui.affectsDisplay:echo("Affects", "\n")
	end
