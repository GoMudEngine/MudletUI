...
function ui.installGoMudUI()
	...
	-- Debug: Check GMCP before update
	ui.debugGMCP("installGoMudUI START")

	ui.isUpdating = true
	
	-- Debug: Check GMCP before uninstall
	ui.debugGMCP("installGoMudUI BEFORE UNINSTALL")
	uninstallPackage("GoMudUI")
	
	-- Debug: Check GMCP after uninstall
	tempTimer(0.5, function()
		ui.debugGMCP("installGoMudUI AFTER UNINSTALL")
	end)
...
function ui.fetchChangeLog()
	...
	downloadFile(
		ui.gomudUIChangelogFile,
		"https://github.com/GoMudEngine/MudletUI/releases/latest/download/changelog.txt"
	)
end
...
function ui.fileDownloadedSuccess(_, filename)
	...
	if filename == tostring(ui.gomudUIVersionFile) then
		...
		if ui.compare_versions(content, ui.version) then
			...
			ui.displayUIMessage(
				"<magenta>Update found: <sky_blue>" .. ui.version .. "<reset> -> <spring_green>" .. ui.versionNew
			)
	...
	if filename == tostring(ui.gomudUIChangelogFile) then
		...
		cechoLink(
			" <dodger_blue><u>Show the full changelog</u>",
		...
		cechoLink(
			...
			[[ui.installGoMudUI()]],
			"Install new version of GoMud UI",
			true
		)
