function ui.showMapExpLevel()
  ...
  for k, v in spairs(mapTable) do
    ...
    cechoLink("<dark_green>" .. areaName, [[ui.showUnvisitedRooms("]] .. v .. [[")]],
      string.trim(areaName) .. " - show rooms", true)
    cecho("<lime_green>" ..
    visitedRooms .. " <light_blue>/ <gold>" .. areaRooms .. "   <dodger_blue>" .. visitedPercentage .. " <grey>%")
    cechoPopup("<DimGray>  (Mark all)\n", { [[ui.markAreaVisited("]] .. v .. [[")]], [[ui.clearAreaVisited("]] ..
    v .. [[")]] }, { string.trim(areaName) .. " - mark area visited", string.trim(areaName) .. " - mark area not visited" },
      true)
  end

  if getRoomArea(gmcp.Room.Info.num) then
    cecho("\n\n<lime_green>Click here to ")
    cechoLink("<firebrick>highlight ", [[ui.highlightUnvisitedRooms(]] .. getRoomArea(gmcp.Room.Info.num) .. [[)]], "",
      true)
    cecho("<lime_green>or ")
    cechoLink("<light_blue>unhighlight ",
      [[ui.highlightUnvisitedRooms(]] .. getRoomArea(gmcp.Room.Info.num) .. [[,"unhighlight")]], "", true)
