...
local function include(color, options)
    -- Always exclude LightGoldenrodYellow
    if color == "LightGoldenrodYellow" then
        return false
    end
...
local function echoColor(color, options, isLastInRow)
    local rgb = color.rgb
    local fgc = "white"
    ...
    local colorString
    -- Keep fixed width for alignment, but reduce from 23 to 18 characters
    -- Add 2 spaces between columns except for the last one
    if options.justText then
        colorString = string.format('<%s:%s> %-18s<reset>', color.name, 'black', color.name)
    else
        colorString = string.format('<%s:%s> %-18s<reset>', fgc, color.name, color.name)
    end
    -- Add spacing after the color unless it's the last in the row
    ...
    if options.window == "main" then
        if options.echoOnly then
            ...
            cechoLink(
                colorString,
                function()
                    ui.settings[options.uiSetting] = color.name
                    ui.addCSSToSettings()
...
function displayColors(options)
    ...
    options.cols = options.cols or 3
    ...
    if options.columnSort then
        ...
        for i = 1, lines do
            for j = 1, cols do
                ...
                if color then
                    local isLastInRow = (j == cols)
                    echoColor(color, options, isLastInRow)
                end
            end
            echo(options.window, "\n")
        end
        ...
        for _, k in ipairs(colors) do
            local isLastInRow = (i == cols)
            echoColor(k, options, isLastInRow)
            if i == cols then
                echo(options.window, "\n")
                i = 1
            else
