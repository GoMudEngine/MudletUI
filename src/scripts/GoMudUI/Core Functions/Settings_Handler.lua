	...
	if tabNew == "Settings" then
		...
		for _, key in pairs(setKeys) do
			...
			for k, v in pairs(ui.settings.userToggles[key]) do
				...
				ui.mapperDisplay:cechoLink(
					"Settings",
					"<white>] " .. ui.settings.userToggles[key][k].desc .. "\n",
					[[ui.settingsHandler("]] .. key .. [[","]] .. k .. [[")]],
					ui.settings.userToggles[key][k].desc,
					true
				)
			end
		end
