...
When developer mode is enabled:
  ...
  <yellow>ui dev checkgmcp<reset>           Check current GMCP state
  <yellow>ui dev gmcpwatch start<reset>     Start GMCP state monitoring
  <yellow>ui dev gmcpwatch stop<reset>      Stop GMCP state monitoring
	...
	elseif subCommand == "export" then
		ui.dev.export()
	elseif subCommand == "checkgmcp" then
		-- Add a simple GMCP check command that doesn't require dev mode
		ui.debugGMCP("Manual check via ui dev checkgmcp")
		if gmcp and gmcp.Char then
			cecho("\n<green>Sample GMCP data:<reset>\n")
			if gmcp.Char.Vitals then
				cecho("  <yellow>Char.Vitals.hp:<reset> " .. tostring(gmcp.Char.Vitals.hp or "nil") .. "\n")
				cecho("  <yellow>Char.Vitals.mp:<reset> " .. tostring(gmcp.Char.Vitals.mp or "nil") .. "\n")
			end
			if gmcp.Char.Stats then
				cecho("  <yellow>Char.Stats.name:<reset> " .. tostring(gmcp.Char.Stats.name or "nil") .. "\n")
			end
			if gmcp.Room and gmcp.Room.Info then
				cecho("  <yellow>Room.Info.num:<reset> " .. tostring(gmcp.Room.Info.num or "nil") .. "\n")
			end
		end
	elseif subCommand == "gmcpwatch" then
		-- Handle GMCP watcher commands
		if ui.debug and ui.debug.handleWatcherCommand then
			ui.debug.handleWatcherCommand(args)
		else
			ui.displayUIMessage("GMCP watcher not initialized")
		end
