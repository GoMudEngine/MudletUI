...
function ui.dev.testDisplay(displayName)
	...
	-- WARNING: We should not modify gmcp directly
	ui.displayUIMessage("<red>ERROR: testDisplay function needs to be rewritten to not modify gmcp directly<reset>")
	ui.displayUIMessage("This function is currently disabled to prevent GMCP corruption")
	
	-- TODO: Rewrite this to use a mock data approach that doesn't touch the global gmcp
	-- Possible solutions:
	-- 1. Create a ui.mockGmcp flag and check it in getGmcpData
	-- 2. Override the display update functions temporarily
	-- 3. Create test-specific display instances
end
