    ...
    if not table.contains(getPackages(), "GoMudMapper") then
      ...
      tempTimer(1,
        function() installPackage(
          "https://github.com/GoMudEngine/MudletMapper/releases/latest/download/GoMudMapper.mpackage") end)
    end
...
function ui.unInstall(_, package)
  if package == "GoMudUI" and not ui.isUpdating then
    ...
    if not table.contains(getPackages(), "generic_mapper") then
      tempTimer(1,
        function() installPackage(
          "https://raw.githubusercontent.com/Mudlet/Mudlet/development/src/mudlet-lua/lua/generic-mapper/generic_mapper.xml") end)
    end
