...
function ui.showContainerState()
	...
	for k, v in spairs(ui.settings.containers) do
		local state
		if ui[k].hidden then
			state = "<firebrick>Hidden<reset>"
		else
			state = "<ForestGreen>Shown<reset>"
		end
		cecho("  " .. k .. ":" .. string.rep(" ", 18 - string.len(k)) .. state)
		cecho(string.rep(" ", 10 - string.len(cecho2string(state))))
		cechoLink("<u>Show</u>", function()
			ui[k]:show()
			ui.showContainerState()
		end, "Show container", true)
		cechoLink("    <u>Hide</u>", function()
			ui[k]:hide()
			ui.showContainerState()
		end, "Hide container", true)
		cechoLink("    <u>Flash</u>", function()
			ui[k]:flash()
		end, "Identify the container", true)
		echo("\n")
		echo("\n")
	end
	echo("\n")
