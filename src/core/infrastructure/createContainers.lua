function ui.createContainers(arg)
	...
	ui.top = ui.top
		or Adjustable.Container:new({
			name = "ui.top",
			y = 0,
			height = "4c",
			padding = 4,
			attachedMargin = 0,
			adjLabelstyle = ui.getStyle("containers", "top"),
	...
	ui.bottom = ui.bottom
		or Adjustable.Container:new({
			...
			padding = 4,
			attachedMargin = 0,
			adjLabelstyle = ui.getStyle("containers", "bottom"),
			autoLoad = containerAutoload,
		})
	...
	ui.right = ui.right
		or Adjustable.Container:new({
			...
			padding = 4,
			attachedMargin = 0,
			adjLabelstyle = ui.getStyle("containers", "right"),
			autoLoad = containerAutoload,
		})
	...
	ui.left = ui.left
		or Adjustable.Container:new({
			...
			padding = 4,
			attachedMargin = 10,
			adjLabelstyle = ui.getStyle("containers", "left"),
			autoLoad = containerAutoload,
		})
	...
	for k, v in pairs(ui.settings.containers) do
		...
		ui[k] = ui[k]
			or Adjustable.Container:new({
				...
				adjLabelstyle = v.customCSS or ui.getStyle("containers", "moveable"),
				autoLoad = containerAutoload,
			}, ui[v.dest])
	...
	for k, v in pairs(ui.settings.displays) do
		...
		elseif v.mapper then
			...
			ui[k] = Geyser.MiniConsole:new({
				...
				font = ui.settings.consoleFont,
				adjLabelstyle = ui.getStyle("containers", "noBorder"),
				autoWrap = false,
