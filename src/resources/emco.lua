...
function EMCO:readYATCO()
    ...
    constraints = string.format(
        '%s  activeTabBGColor = "<%s,%s,%s>",\n',
        constraints,
        config.activeColors.r,
        config.activeColors.g,
        config.activeColors.b
    )
    constraints = string.format(
        '%s  inactiveTabBGColor = "<%s,%s,%s>",\n',
        constraints,
        config.inactiveColors.r,
        config.inactiveColors.g,
        config.inactiveColors.b
    )
    constraints = string.format(
        '%s  consoleColor = "<%s,%s,%s>",\n',
        constraints,
        config.windowColors.r,
        config.windowColors.g,
...
function EMCO:enableCmdLine(tabName, template)
    if not table.contains(self.consoles, tabName) then
        return nil,
            f(
                "{self.name}:enableCmdLine(tabName,template) tabName is not in the console list. Valid options are {table.concat(self.consoles, 'm')}"
            )
    end
    local window = self.mc[tabName]
...
function EMCO:disableCmdLine(tabName)
    if not table.contains(self.consoles, tabName) then
        return nil,
            f(
                "{self.name}:disableCmdLine(tabName,template) tabName is not in the console list. Valid options are {table.concat(self.consoles, 'm')}"
            )
    end
    local window = self.mc[tabName]
...
function EMCO:createContainers()
    ...
    self.tabBox = Geyser.HBox:new(
        { x = 0, y = 0, width = "100%", height = "100%", name = self.name .. "TabBox" },
        self.tabBoxLabel
    )
...
function EMCO:setFont(font)
    ...
    if not (af[font] or font == "") then
        local err = "EMCO:setFont(font): attempt to call setFont with font '"
            .. font
...
function EMCO:setSingleWindowFont(tabName, font)
    ...
    local af = getAvailableFonts()
    if not (af[font] or font == "") then
        local err = "EMCO:setSingleWindowFont(tabName, font): attempt to call setFont with font '"
            .. font
            .. "' which is not available, see getAvailableFonts() for valid options\n"
        err = err
            .. "In the meantime, we will use a similar font which isn't the one you asked for but we hope is close enough"
...
function EMCO:setAllTabName(allTabName)
    ...
    if not table.contains(self.consoles, allTabName) then
        self.ae(
            funcName,
            "allTabName must be the name of one of the console tabs. Valid options are: "
...
function EMCO:setMapTabName(mapTabName)
    ...
    if not table.contains(self.consoles, mapTabName) and mapTabName ~= "" then
        self.ae(
            funcName,
            "mapTabName must be one of the existing console tabs. Current tabs are: "
            .. table.concat(self.consoles, ",")
        )
    end
...
function EMCO:append(tabName, excludeAll)
    ...
    elseif not validTab then
        self.ae(
            funcName,
            "tabName must be a tab which is contained in this object. Valid tabnames are: "
            .. table.concat(self.consoles, ",")
        )
    end
...
function EMCO:xEcho(tabName, message, xtype, excludeAll)
    ...
    local allTab = (
            self.allTab
            and not excludeAll
            and not table.contains(self.allTabExclusions, tabName)
            and tabName ~= self.allTabName
        )
        and self.mc[self.allTabName]
        or false
...
function EMCO:xLink(tabName, linkType, text, commands, hints, useCurrentFormat, excludeAll)
    ...
    if gag then
        debugc(
            f("{self.name}:{linkType}(tabName, text, command, hint, excludeAll) denied because text matches '{reason}'")
        )
        return
    end
    local console = self.mc[tabName]
...
function EMCO:load()
    ...
    if io.exists(filename) then
        ...
        debugc(
            string.format(
                "Attempted to load config for EMCO named %s but the file could not be found. Filename: %s",
                self.name,
                filename
            )
        )
...
function EMCO:enableTabLogging(tabName)
    ...
    if logDisabled then
        table.remove(self.logExclusions, logDisabled)
    end
end
...
function EMCO:disableTabLogging(tabName)
    ...
    if not logDisabled then
        table.insert(self.logExclusions, tabName)
    end
end
