...
function ftext.fLine(str, opts)
	...
	local fullLeftSpacer = string.format(
		"%s%s%s",
		options.spacerColor,
	...
	local fullRightSpacer = string.format(
		"%s%s%s",
		options.spacerColor,
	...
	if options.inside then
		...
		-- "some text =================endcap"
		local finalString =
			string.format("%s%s%s%s%s", fullLeftCap, fullLeftSpacer, fullText, fullRightSpacer, fullRightCap)
		return finalString
	else
		-- "=====endcap some text endcap====="
		-- "=====endcap some text pacdne====="
		...
		local finalString =
			string.format("%s%s%s%s%s", fullLeftSpacer, fullLeftCap, fullText, fullRightCap, fullRightSpacer)
		return finalString
	end
end
...
local TextFormatter = {}
TextFormatter.validFormatTypes = {
	"d",
	"dec",
	"decimal",
	"h",
...
function TextFormatter:setWrap(shouldWrap)
	...
	if shouldWrap == nil then
		error(
			"TextFormatter:setWrap(shouldWrap) Argument error, boolean expected, got "
				.. argumentType
...
function TableMaker:addColumn(options, position)
	...
	if not type(options) == "table" then
		error(
			"TableMaker:addColumn(options, position): Argument error: options expected as table, got " .. type(options)
		)
	end
...
function TableMaker:replaceColumn(options, position)
	...
	if type(options) ~= "table" then
		error(
			"TableMaker:replaceColumn(options, position): Argument error: options as table expected, got "
				.. type(options)
		)
	end
...
function TableMaker:addRow(columnEntries, position)
	...
	if columnEntriesType ~= "table" then
		error(
			"TableMaker:addRow(columnEntries, position): Argument error, columnEntries expected as table, got "
				.. columnEntriesType
		)
	end
	for _, entry in ipairs(columnEntries) do
		...
		if entryCheck == 0 then
			if type(entry) == "function" then
				...
				error(
					"TableMaker:addRow(columnEntries, position): Argument error, columnEntries items expected as string, got:"
						.. type(entry)
				)
			end
		end
...
function TableMaker:replaceRow(columnEntries, position)
	if position == nil then
		error(
			"TableMaker:replaceRow(columnEntries, position): ArgumentError: position expected as number, received nil"
		)
	end
...
function TableMaker:makeHeader()
	...
	if self.printHeaders then
		...
		columnHeaders = string.format(
			"\n%s%s%s%s",
			ec,
			table.concat(columnEntries, sep),
			ec,
...
function TableMaker:createRowDivider()
	...
	for _, v in ipairs(self.columns) do
		local piece = string.format(
			"%s%s%s",
			self.separatorColor,
			string.rep(self.rowSeparator, v.options.width),
			self.colorReset
		)
...
function TableMaker:setAutoEchoConsole(console)
	...
	if consoleType ~= "string" and consoleType ~= "table" then
		error(
			funcName
				.. " ArgumentError: console as string or a Geyser MiniConsole or UserWindow expected, got "
				.. consoleType
		)
...
function TableMaker:textAssemble()
	...
	local footer =
		string.format("%s%s%s", self.frameColor, string.rep(self.footCharacter, self:totalWidth()), self.colorReset)
	sheet = string.format(
		"%s\n%s\n%s\n",
		self:makeHeader(),
...
function TableMaker:new(options)
	...
	if options.columns then
		if type(options.columns) ~= "table" then
			error(
				"TableMaker:new(options): option error: You provided an options.columns entry of type "
	...
	if options.rows then
		if type(options.rows) ~= "table" then
			error(
				"TableMaker:new(options): option error: You provided an options.rows entry of type "
					.. type(options.rows)
